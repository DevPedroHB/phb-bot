  ...
  async run({ interaction }) {
    ...
    if (channel?.type !== ChannelType.GuildText) {
      return interaction.reply({
        ephemeral: true,
        content: "Este comando não pode ser utilizado neste canal!",
      });
    }
    ...
    const message = await interaction.reply({
      ...
      components: [
        new ActionRowBuilder<ButtonBuilder>({
          components: [
            new ButtonBuilder({
              customId: "questions-start-button",
              label: "Começar",
    ...
    const quest = async (steps = 0) => {
      if (steps < questions.length) {
        ...
        if (msg.content.toLowerCase() == "cancelar") {
          embed.setDescription("O questionário foi cancelado!").setFooter(null);
      ...
      embed
        .setDescription("Todas as respostas foram registradas! 📝")
        .setFields(
          answers.map(({ title, answer }) => ({ name: title, value: answer }))
        )
        .setFooter(null);
